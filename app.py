...
def display_norm_price_chart(market_df: pd.DataFrame, title_prefix: str):
    ...
    for product in products_to_plot:
        ...
        if vwap_needs_calc:
            ...
            try:
                 ...
                 # Apply row-wise ON THE ISOLATED product_df
                 ...
                 if not product_df['vwap_recalculated'].isna().all():
                      ...
                      print(f"    Recalculated VWAP all NaN for {product}.")
            except Exception as e:
        ...
        if price_col_to_use:
            ...
            # Select only needed cols for this product
            product_norm_data = product_df[['timestamp', 'product', price_col_to_use]].copy()
            product_norm_data.dropna(subset=[price_col_to_use], inplace=True) # Drop only if the price col is NaN
            ...
            if not product_norm_data.empty:
                ...
                # Append the processed data for this product (including price_change)
    ...
    # Combine the normalized dataframes for all products
    ...
    # Determine overall Y-axis title based on what columns were used primarily
    ...
    try:
        ...
        pad = (max_y - min_y) * 0.05 + 1 # Add padding
...
def display_fill_dist_chart(market_df: pd.DataFrame, trades_df: pd.DataFrame, title_prefix: str, is_permutation_run: bool = False):
    ...
    if not vwap_available:
        try: # Calculate VWAP on the *whole* market df for merging with trades
            price_vol_cols=[f'{s}_{t}_{l}' for s in ['bid','ask'] for t in ['price','volume'] for l in [1,2,3]]
            for col in price_vol_cols:
    ...
    if player_trades.empty:
        st.info(f"Fill Dist Chart: No SUBMISSION trades ({title_prefix}).")
        return
    def get_fill_type(row):
        if row['buyer']=='SUBMISSION':
            return row.get('seller', 'Unk')
        else:
            return row.get('buyer', 'Unk')
    player_trades['fill_type'] = player_trades.apply(get_fill_type, axis=1)
    expected_fills = ['ExplicitBook', 'InferredBot'] # Allow both sources now for perm runs too
    player_trades = player_trades[player_trades['fill_type'].isin(expected_fills)].copy()
    if player_trades.empty:
        st.info(f"Fill Dist Chart: No trades with types {expected_fills} ({title_prefix}).")
        return
    ...
    try:
        ...
        if combined_summary.empty:
            ...
            return
        color_map = {"ExplicitBook": px.colors.qualitative.Plotly[0], "InferredBot": px.colors.qualitative.Plotly[1], "Unknown": "grey", "Unk": "grey"} # Added Unk just in case
...
def rerun_and_view_permutation(perm_index_to_view: int, perm_args: dict):
    ...
    # ... (Add more inspection prints if needed) ...
...
with st.sidebar:
    ...
    selected_trader_fname = None; selected_data_fnames = []; selected_log_viewer_file = None
    perm_blocksize = 10; perm_n_iterations = 99; time_range_values = (0, constants.MAX_TIMESTAMP)
    bot_behavior = 'lte'; ignore_limits_checkbox = False; run_permutation_test = False
    ...
    if not is_viewer_mode: # Backtest Mode UI
        ...
        else: default_selection = [data_files[0]] if data_files else None; selected_data_fnames = st.multiselect("Log Files (first primary):", data_files, default=default_selection, key="data_select", label_visibility="collapsed")
        st.subheader("3. Settings"); col_a, col_b = st.columns(2)
        with col_a: bot_behavior = st.selectbox("Bot Matching:", ["none", "eq", "lt", "lte"], index=3, key="bot_behavior", help="Rule vs inferred bots.")
        with col_b: ignore_limits_checkbox = st.checkbox("Ignore Limits", value=False, key="ignore_limits")
        min_time, max_time = 0, constants.MAX_TIMESTAMP
        if selected_data_fnames:
            try:
                 with open(os.path.join(DATA_DIR_ABS, selected_data_fnames[0]), 'r', encoding='utf-8') as f: _, temp_mkt, _ = util._parse_data(f)
                 if temp_mkt is not None and not temp_mkt.empty and pd.api.types.is_numeric_dtype(temp_mkt.index): min_time, max_time = int(temp_mkt.index.min()), int(temp_mkt.index.max())
            except Exception as e: print(f"Warn: Time range fail {selected_data_fnames[0]}: {e}")
        min_time = max(0, min_time); max_time = min(constants.MAX_TIMESTAMP, max_time) if max_time > min_time else constants.MAX_TIMESTAMP
        ...
        if not perm_utils_available: st.caption("Permutation testing disabled (utils not found).")
        ...
        if run_permutation_test and perm_utils_available:
        ...
        elif run_permutation_test and not perm_utils_available: st.error("Permutation utils failed to import.")
         ...
         if selected_log_viewer_file:
              try:
                   with open(os.path.join(DATA_DIR_ABS, selected_log_viewer_file), 'r', encoding='utf-8') as f: _, temp_mkt, _ = util._parse_data(f)
                   if temp_mkt is not None and not temp_mkt.empty and pd.api.types.is_numeric_dtype(temp_mkt.index): min_time, max_time = int(temp_mkt.index.min()), int(temp_mkt.index.max())
              except Exception as e: print(f"Warn: Viewer time range fail {selected_log_viewer_file}: {e}")
         min_time = max(0, min_time); max_time = min(constants.MAX_TIMESTAMP, max_time) if max_time > min_time else constants.MAX_TIMESTAMP
...
display_source_info = "Configure parameters and run."; data_to_display_available = False

# --- Main Execution Logic ---
if run_button_pressed: # Main execution block when button is pressed
    print("\n--- Run Button Pressed ---"); clear_parsed_data()
    if is_viewer_mode: # Log Viewer Logic
        if selected_log_viewer_file:
            ...
            with st.spinner(f"Parsing **{selected_log_viewer_file}**..."):
                try:
                    ...
                    if raw_content.startswith("Error:"): st.error(raw_content)
                        ...
                        st.session_state.parsed_log_data = (s_logs_f, m_df_f, t_df_f, raw_content); st.success(f"Parsed & filtered **{selected_log_viewer_file}**.")
                except Exception as e: st.error(f"Failed load/parse: {e}"); st.code(traceback.format_exc())
        else: st.warning("Select log file.")
            ...
            try: # Prepare Base Data
                with st.spinner("Analyzing market data..."):
                    ...
                    for product in products_in_run: # Loop products for base data calc
                         ...
                         if 'vwap' not in product_market_data.columns or product_market_data['vwap'].isna().all(): # Calc VWAP if needed
                              ...
                              for col in price_vol_cols:
                                   if col in product_market_data.columns:
                                        if not pd.api.types.is_numeric_dtype(product_market_data[col]):
                                             product_market_data.loc[:, col] = pd.to_numeric(product_market_data[col], errors='coerce')
                         ...
                         rel_explicit_books = perm_utils.calculate_relative_product_books(product_market_data, product_vwap_series)
                            ...
                            if m_df_perm is not None and not m_df_perm.empty and 'profit_and_loss' in m_df_perm.columns:
                                ...
                                if not pnl_s.isna().all():
                                    ...
                                    if last_ts is not None:
                                        ...
                                        final_sum = np.nan # Default if error
                                        if vals is not None:
                                            final_sum = vals.sum() if isinstance(vals, pd.Series) else vals
                                        if pd.notna(final_sum):
                                             current_perm_pnl = float(final_sum)
                                             print(f"Perm {i+1}: PNL OK: {current_perm_pnl}")
    ...
    if not data_to_display_available:
        ...
        if "🧪 Permutation Test" in tab_map: # Permutation Tab
            with tab_map["🧪 Permutation Test"]:
                ...
                valid_perm_pnls = perm_results.get('permuted_pnls', [])
                raw_perm_pnls = perm_results.get('permuted_pnls_raw', [])
                col1, col2 = st.columns(2)
                col1.metric("Original Final PNL", orig_pnl_display)
                col2.metric("p-value", f"{p_val:.4f}" if pd.notna(p_val) else "N/A", help="Prob. PNL >= original.")
                if valid_perm_pnls:
                     ...
                     try:
                          ...
                          st.error(f"Hist Error: {hist_e}")
                          st.error(f"Data: {valid_perm_pnls}")
                          traceback.print_exc()
                else:
                    st.info("No valid perm PNLs to plot.")
                with st.expander(f"View Raw PNLs ({len(raw_perm_pnls)} Runs)"):
                ...
                if max_perms == 0 or not rerun_args:
                    st.info("Run permutation test first.")
                else:
                     ...
                     if output_content_to_use:
                         ...
                         try: # Download Button
                              ...
                              if is_displaying_perm_view:
                                   fname_trader = st.session_state.permutation_rerun_args.get('trader_file','unk').replace('.py','')
                                   fname_log = st.session_state.permutation_rerun_args.get('data_files',['unk'])[0].replace('.log','')
                                   fname_suffix = f"_perm{st.session_state.view_permutation_index}"
                              elif st.session_state.log_viewer_mode:
                                   fname_log = st.session_state.last_loaded_log_file.replace('.log','') if st.session_state.last_loaded_log_file else 'unk'
                              else:
                                   fname_trader = st.session_state.last_run_trader_file.replace('.py','') if st.session_state.last_run_trader_file else 'unk'
                                   fname_log = selected_data_fnames[0].replace('.log','') if selected_data_fnames else 'unk'
                              ...
                              st.warning(f"Download link error: {dl_err}")
                     else:
                         st.warning("No raw output.")

        if "Info" in tab_map:
            with tab_map["Info"]:
                st.info("Configure and run.") # Fallback
