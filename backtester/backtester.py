# --- START OF backtester.py (Style Corrected) ---
...
class Backtester:
    ...
    def __init__(
        ...
        if not hasattr(self, 'products') or not self.products:
            raise ValueError("Products not determined.")
        print(f"DEBUG __init__: Products: {self.products}")
        ...
        if cols_to_drop:
            self.activity_log_output.drop(columns=cols_to_drop, inplace=True, errors='ignore')
        if 'profit_and_loss' not in self.activity_log_output.columns:
            self.activity_log_output['profit_and_loss'] = np.nan
        else:
            # Ensure numeric for calculation, initialize for run
            if not pd.api.types.is_numeric_dtype(self.activity_log_output['profit_and_loss']):
                self.activity_log_output['profit_and_loss'] = pd.to_numeric(self.activity_log_output['profit_and_loss'], errors='coerce')
            self.activity_log_output['profit_and_loss'] = np.nan
    ...
    def _load_and_prepare_data(self):
        ...
        if primary_market_df.empty:
            raise ValueError("Primary market activity data is empty.")
        ...
        if not self.market_data.empty:
            if 'product' in self.market_data.columns:
                all_products.update(p for p in self.market_data['product'].unique() if isinstance(p, str) and p)
            self._precompute_explicit_order_depths(self.market_data)
        else:
            print("Warning: No market data in time range.")
            self.explicit_order_depths_cache = {}
        ...
        for data_path in self.data_paths:
            ...
            try:
                ...
                print(f"Warning: Error parsing {data_path}: {e}. Skipping.")
                continue
            ...
            if not trades_df.empty:
                ...
                if not trades_df_filtered.empty:
                    ...
                    all_trades_list.append(trades_df_filtered) # Store all trades
                        ...
                        for ts, products_data in log_excess_buys.items():
                            for prod, prices_data in products_data.items():
                                for price, vol in prices_data.items():
                                    agg_max_bot_buys[ts][prod][price] = max(agg_max_bot_buys[ts][prod][price], vol)
                        for ts, products_data in log_excess_sells.items():
                            for prod, prices_data in products_data.items():
                                for price, vol in prices_data.items():
                                    agg_max_bot_sells[ts][prod][price] = max(agg_max_bot_sells[ts][prod][price], vol)

                    if 'symbol' in trades_df_filtered.columns:
                        all_products.update(p for p in trades_df_filtered['symbol'].unique() if isinstance(p, str) and p)

        if not all_trades_list:
            print("Warning: No trade history found.")
            self.raw_all_trades = pd.DataFrame()
        else:
             ...
             # More robust duplicate removal considering index
             self.raw_all_trades = self.raw_all_trades.loc[~self.raw_all_trades.index.duplicated(keep='first') | ~self.raw_all_trades.duplicated(subset=key_cols, keep='first')]
        ...
        if not self.products:
            raise ValueError("No products determined.")
        ...
        for ts in timestamps_with_inferred:
            for prod in self.products:
                 has_buys = prod in agg_max_bot_buys[ts] and agg_max_bot_buys[ts][prod]
                 has_sells = prod in agg_max_bot_sells[ts] and agg_max_bot_sells[ts][prod]
    ...
    def _calculate_excess_volume_per_log(self, submission_trades_df, explicit_book_cache):
        log_excess_bot_buys = defaultdict(lambda: defaultdict(Counter))
        log_excess_bot_sells = defaultdict(lambda: defaultdict(Counter))
        ...
        for (timestamp, product), trades in grouped_trades:
            ...
            if sub_buys_at_price: # Simulate Buys vs Explicit Sells
                ...
                for sub_p in sorted(sub_buys_at_price.keys()):
                    ...
                    for book_p in sorted(explicit_sells_copy.keys()):
                        ...
                        filled = min(vol_match, book_v)
                        if filled > 0:
                            fills[sub_p] += filled
                            explicit_sells_copy[book_p] += filled # Consume volume
                            vol_match -= filled
                            if explicit_sells_copy[book_p] == 0:
                                del explicit_sells_copy[book_p]
                    excess = sub_buys_at_price[sub_p] - fills[sub_p]
                    if excess > 0:
                        log_excess_bot_sells[timestamp][product][sub_p] = excess

            if sub_sells_at_price: # Simulate Sells vs Explicit Buys
    ...
    def _precompute_explicit_order_depths(self, market_df: pd.DataFrame):
        ...
        self.explicit_order_depths_cache = {}
        if market_df.empty:
            print("Warning: Market data empty, cannot precompute depths.")
            return
        if not pd.api.types.is_numeric_dtype(market_df.index):
            try:
                # Attempt conversion if not numeric, e.g., from object
                market_df.index = pd.to_numeric(market_df.index)
                print("Converted market data index to numeric for grouping.")
            except Exception as e:
                print(f"ERROR: Cannot convert market data index to numeric: {e}")
                return

        try:
            grouped = market_df.groupby(market_df.index)
        except Exception as e:
             print(f"ERROR: Failed to group market_df by index: {e}")
             return
        ...
        for timestamp, group in grouped:
            ...
            for product in products_in_group:
                ...
                product_rows = group.loc[group['product'] == product]
                if product_rows.empty:
                    continue
                product_row = product_rows.iloc[0]

                for i in range(1, 4): # Buys
                    bp, bv = f'bid_price_{i}', f'bid_volume_{i}'
                    # Check column exists in index (for Series) or columns (for DataFrame)
                    if bp in product_row.index and bv in product_row.index and pd.notna(product_row[bp]) and pd.notna(product_row[bv]) and product_row[bv] > 0:
                        try:
                            order_depth.buy_orders[int(product_row[bp])] = int(product_row[bv])
                        except (ValueError, TypeError):
                            # print(f"Warn: Invalid buy data {product}@{timestamp}") # Reduce noise
                            pass
                for i in range(1, 4): # Sells
                    ap, av = f'ask_price_{i}', f'ask_volume_{i}'
                    if ap in product_row.index and av in product_row.index and pd.notna(product_row[ap]) and pd.notna(product_row[av]) and product_row[av] > 0:
                        try:
                            order_depth.sell_orders[int(product_row[ap])] = -int(product_row[av])
                        except (ValueError, TypeError):
                            # print(f"Warn: Invalid sell data {product}@{timestamp}") # Reduce noise
                            pass
                # Only add if there's actual depth
                if order_depth.buy_orders or order_depth.sell_orders:
                    ts_depths[product] = order_depth
            # Only add timestamp if there was depth for any product
            if ts_depths:
    ...
    def run(self, order_depth_override_cache: Optional[Dict] = None, disable_inferred_book: bool = False):
        run_mode = "OVERRIDE" if order_depth_override_cache is not None else "NORMAL"
        print(f"--- Starting simulation run ({run_mode}) ---")
        print(f"Disable inferred book: {disable_inferred_book}")
        if order_depth_override_cache is not None:
            print(f"Override cache provided with {len(order_depth_override_cache)} timestamps.")
        else:
            print("Using original explicit and inferred caches.")
        ...
        self.all_trades_log_output = []
        self.last_processed_timestamp = -1
        # Always start with a clean copy of the base market data structure for output logging
        self.activity_log_output = self.market_data.copy()
        cols_to_drop = [col for col in self.activity_log_output.columns if col.startswith('pnl_')]
        if cols_to_drop:
            self.activity_log_output.drop(columns=cols_to_drop, inplace=True, errors='ignore')
        # Ensure profit_and_loss exists and is NaN
        if 'profit_and_loss' not in self.activity_log_output.columns:
            self.activity_log_output['profit_and_loss'] = np.nan
        else:
             if not pd.api.types.is_numeric_dtype(self.activity_log_output['profit_and_loss']):
                 self.activity_log_output['profit_and_loss'] = pd.to_numeric(self.activity_log_output['profit_and_loss'], errors='coerce')
             self.activity_log_output['profit_and_loss'] = np.nan # Set to NaN for this run
        ...
        if run_mode == "OVERRIDE":
            if order_depth_override_cache is None: # Should not happen if run_mode is OVERRIDE
                 print("ERROR: Override mode selected but override cache is None!")
                 active_explicit_book_cache = self.explicit_order_depths_cache # Fallback
            else:
                 active_explicit_book_cache = order_depth_override_cache
            use_inferred_book = False
            active_inferred_cache = {}
            active_fv_book_cache = active_explicit_book_cache # Use override for FV too
            print("RUN MODE: OVERRIDE - Using override cache. Inferred book disabled.")
        else: # NORMAL mode
            ...
            active_fv_book_cache = self.explicit_order_depths_cache # Use original explicit for FV
            print(f"RUN MODE: NORMAL - Using original explicit cache. Inferred book usage: {use_inferred_book}")

        if not active_explicit_book_cache:
             print("ERROR: The active explicit book cache is empty or None. Aborting run.")
             self._finalize_activity_log()
             self._generate_output()
             return

        timestamps_to_iterate = sorted([ts for ts in active_explicit_book_cache.keys() if self.start_time <= ts <= self.end_time])

        if not timestamps_to_iterate:
            print(f"Warning: No timestamps found in active book cache within time range ({self.start_time}-{self.end_time}).")
            self._finalize_activity_log()
            self._generate_output()
            return
        print(f"Simulating {len(timestamps_to_iterate)} timestamps from active book cache...")
        # --- End Determine ---
        ...
        for i, timestamp in enumerate(timestamps_to_iterate):
            ...
            try:
                if hasattr(self, 'raw_all_trades') and not self.raw_all_trades.empty:
                    ...
                    try:
                        ...
                        pass # Range not in index
            except Exception as e:
                print(f"Error slicing self.raw_all_trades @{timestamp}: {e}")

            # Prepare state using the *active* explicit book
            ...
            state = TradingState(traderData, timestamp, self.listings, current_depths_for_state, {}, dict(current_step_market_trades_dict), self.current_position.copy(), Observation({}, {}))
            ...
            bot_liquidity_books_at_t = copy.deepcopy(active_inferred_cache.get(timestamp, {}))
            ...
            if trader_orders_dict:
                ...
                for product, orders in trader_orders_dict.items():
                   if product in self.listings and isinstance(orders, list):
                        for order in orders:
                            if isinstance(order, Order) and isinstance(order.price, int) and isinstance(order.quantity, int):
                                all_submitted_orders.append(order)
                            # else: Log warning about invalid order?
                ...
                for product, product_orders in grouped_player_orders.items():
                    ...
                    if not self.ignore_limits and product in self.position_limit:
                        ...
                        total_sell = sum(abs(o.quantity) for o in product_orders if o.quantity < 0)
                        curr = self.current_position.get(product, 0)
                        limit = self.position_limit.get(product, 0)
                        if (curr + total_buy > limit) or (curr - total_sell < -limit):
                            limit_breached = True
                    ...
                    if not limit_breached:
                        for order in product_orders:
                            ...
                            # 1. Match Explicit Book
                            trades_explicit, sandbox_log_output = self._match_against_book(timestamp, order_copy, explicit_depths_for_matching, sandbox_log_output, "ExplicitBook", "any")
                            executed_own_trades_this_step[product].extend(trades_explicit)
                            # 2. Match Inferred Bot Liquidity (if enabled and available)
                            if use_inferred_book and abs(order_copy.quantity) > 0 and product in bot_liquidity_books_at_t:
                                 ...
                                 trades_bot, sandbox_log_output = self._match_against_book(timestamp, order_copy, current_prod_inferred, sandbox_log_output, "InferredBot", self.bot_behavior)
            ...
            # Log non-player trades for context (Optional, using self.raw_all_trades)
            if hasattr(self, 'raw_all_trades') and not self.raw_all_trades.empty:
                 ...
                 for _, row in non_player_trades_at_t.iterrows():
                     try: # Add extra safety for type conversion
                         mtrade = Trade(row['symbol'], int(row['price']), int(row['quantity']), row['buyer'], row['seller'], timestamp)
                         self.all_trades_log_output.append(self._trade_to_dict(mtrade))
                     except Exception as trade_err:
                         print(f"Warn: Failed to process non-player trade at {timestamp}: {trade_err}, row: {row}")


            self._calculate_and_log_pnl(timestamp, active_fv_book_cache) # Use active book for FV calc
        ...
        if hasattr(self, 'activity_log_output') and not self.activity_log_output.empty:
            print(f"Activity Log Tail:\n{self.activity_log_output.tail(10)}")
        self._finalize_activity_log()
        self._generate_output()


    def _match_against_book(self, timestamp: int, order: Order, order_books: Dict[Symbol, OrderDepth], log: str, book_type: str, match_behavior: Literal["none", "eq", "lt", "lte", "any"]) -> Tuple[List[Trade], str]:
        trades_made = []
        product = order.symbol
        # Modify the passed dictionary directly to track consumed liquidity within the timestamp
        book = order_books.get(product)
        if not book:
            return trades_made, log

        if order.quantity > 0:  # Buy Order
            # Iterate over a copy of keys to allow deletion from original dict
            sell_prices = sorted([int(p) for p in book.sell_orders.keys()])
            for price in sell_prices:
                if order.quantity <= 0: break
                if price > order.price: continue # Basic price check
                ...
                if book_type == "InferredBot" and match_behavior != "any":
                    ...
                    elif match_behavior == "lt": allowed = (price < order.price)
                    elif match_behavior == "lte": allowed = (price <= order.price)
                    if not allowed: continue

                # Check if price still exists in the original dict
                ...
                book.sell_orders[price] += vol_to_trade # Consume volume
            ...
            for price in buy_prices:
                ...
                if book_type == "InferredBot" and match_behavior != "any":
                    allowed = False
                    if match_behavior == "eq": allowed = (price == order.price)
                    elif match_behavior == "lt": allowed = (price > order.price) # Flipped
                    elif match_behavior == "lte": allowed = (price >= order.price) # Flipped
                    if not allowed: continue
                ...
                self.cash[product] = self.cash.get(product, 0.0) + (price * vol_to_trade)
                order.quantity += vol_to_trade
                book.buy_orders[price] -= vol_to_trade # Consume volume
                if book.buy_orders[price] == 0:
                    del book.buy_orders[price]
        return trades_made, log


    def _calculate_and_log_pnl(self, timestamp: int, book_cache_for_fv: dict):
         total_pnl_at_t = 0.0
         # Ensure index is valid before attempting operations
         if not isinstance(self.activity_log_output.index, pd.Index):
              print(f"ERROR PNL @{timestamp}: Invalid index type for activity_log_output.")
              return
         if not self.activity_log_output.index.is_monotonic_increasing:
              try: self.activity_log_output.sort_index(inplace=True)
              except Exception as sort_err: print(f"ERROR PNL @{timestamp}: Failed to sort index: {sort_err}"); return # Avoid proceeding with bad index

         for product in self.products:
              calculated_pnl = self.pnl.get(product, 0.0) # Use last known PNL as default
              if product in self.fair_price_calc and product in self.current_position:
                  ...
                  if pd.isna(fair_value):
                      try: # Ffill logic - ensure index exists and slice safely
                           ...
                           # else: timestamp not in index, cannot ffill based on log easily
                      except Exception as ffill_err: print(f"Error ffill FV {product}@{timestamp}: {ffill_err}") # Be more specific

                  current_cash=self.cash.get(product,0.0)
                  current_pos=self.current_position.get(product,0)
                  if not pd.isna(fair_value): # Update PNL only if FV is valid
                      calculated_pnl = current_cash + (current_pos * fair_value)

                  # Update internal state regardless of whether calculation updated pnl this step
                  self.pnl[product] = calculated_pnl
                  self.pnl_over_time[product].append((timestamp, calculated_pnl))

              # Log product-specific pnl to profit_and_loss column
              try:
                 if timestamp in self.activity_log_output.index: # Check timestamp exists before .loc
                      ...
                      if row_mask_product.any():
                          if 'profit_and_loss' in self.activity_log_output.columns:
                              self.activity_log_output.loc[row_mask_product, 'profit_and_loss'] = calculated_pnl
                      # else: No row for this product/ts in the *original* market data structure - this is okay.
                 # else: Timestamp missing from original market data index - cannot log here.
              except Exception as log_err: print(f"Error logging PNL {product}@{timestamp}: {log_err}")


              total_pnl_at_t += calculated_pnl # Add current product PNL (calculated or last known)
    ...
    def _finalize_activity_log(self):
        ...
        if not hasattr(self, 'activity_log_output') or not isinstance(self.activity_log_output, pd.DataFrame) or self.activity_log_output.empty:
            print("ERROR finalize: activity_log_output missing, not DataFrame, or empty.")
        ...
        # Final check/drop of stray columns
        unexpected_pnl_cols = [col for col in self.final_activity_log.columns if col.startswith('pnl_')]
        if unexpected_pnl_cols:
            print(f"WARN finalize: Dropping unexpected pnl_ columns: {unexpected_pnl_cols}")
            self.final_activity_log.drop(columns=unexpected_pnl_cols, inplace=True, errors='ignore')

        if pnl_col in self.final_activity_log.columns and 'product' in self.final_activity_log.columns:
            print(f"DEBUG finalize: Processing '{pnl_col}' (product-specific PNL).")
            # Ensure numeric
            ...
            # Ffill *grouped by product*
            self.final_activity_log[pnl_col] = self.final_activity_log.groupby('product', group_keys=False)[pnl_col].ffill()
            # Fill remaining NaNs with 0
            self.final_activity_log[pnl_col] = self.final_activity_log[pnl_col].fillna(0)
            print(f"DEBUG finalize: '{pnl_col}' processing complete. NaN count: {self.final_activity_log[pnl_col].isna().sum()}")
        elif pnl_col not in self.final_activity_log.columns:
            print(f"ERROR finalize: '{pnl_col}' column not found! Adding column with 0s.")
            self.final_activity_log[pnl_col] = 0.0
        # else: product column missing, cannot group
        ...
        final_standard_cols = [col for col in standard_cols_order if col in self.final_activity_log.columns]
        processed_cols = set(final_standard_cols + ['day'])
        other_cols = sorted([col for col in self.final_activity_log.columns if col not in processed_cols and not col.startswith('pnl_')])
        final_col_order = []
        if 'day' in self.final_activity_log.columns: final_col_order.append('day')
        final_col_order.extend([col for col in final_standard_cols if col != 'day'])
        final_col_order.extend(other_cols)
        # Select only columns that actually exist to avoid KeyErrors
        final_col_order = [col for col in final_col_order if col in self.final_activity_log.columns]
        try:
             self.final_activity_log = self.final_activity_log[final_col_order]
        except KeyError as e:
             print(f"ERROR finalize: KeyError during column reordering: {e}. Using available columns.")
             self.final_activity_log = self.final_activity_log[[col for col in self.final_activity_log.columns if not col.startswith('pnl_')]] # Fallback
    ...
    def _generate_output(self):
        ...
        for i, log_entry in enumerate(self.sandbox_logs_capture): # Safer dump
            try:
                safe_entry = {"sandboxLog": str(log_entry.get("sandboxLog", "")),"lambdaLog": str(log_entry.get("lambdaLog", "")),"timestamp": int(log_entry.get("timestamp", -1))}
                json.dump(safe_entry, output_stream)
                output_stream.write("\n")
            except Exception as e:
                print(f"Error dumping sandbox log {i}: {e}")
                output_stream.write('{"error": "sandbox log dump failed"}\n')

        output_stream.write("\n\nActivities log:\n")
        header = "day;timestamp;product;bid_price_1;bid_volume_1;ask_price_1;ask_volume_1;mid_price;profit_and_loss\n" # Basic header
        if hasattr(self, 'final_activity_log') and isinstance(self.final_activity_log, pd.DataFrame) and not self.final_activity_log.empty:
            try:
                # Check dtypes right before writing
                print("DEBUG generate: Dtypes before to_csv:\n", self.final_activity_log.dtypes)
                self.final_activity_log.to_csv(output_stream, sep=';', index=True, float_format='%.6f', header=True)
            except Exception as e:
                print(f"ERROR generate: Failed write activity log: {e}")
                traceback.print_exc()
                output_stream.write(header + f"ERROR writing activity log: {e}\n")
        else:
             print("Warning generate: Final activity log missing or empty.")
             output_stream.write(header)

        output_stream.write("\n\n\nTrade History:\n")
        try: # Safer dump
            safe_trades = []
            for i, trade_dict in enumerate(self.all_trades_log_output):
                 try:
                      safe_trade = {"timestamp": int(trade_dict.get('timestamp', 0)),"buyer": str(trade_dict.get('buyer', '')),"seller": str(trade_dict.get('seller', '')),"symbol": str(trade_dict.get('symbol', '')),"currency": str(trade_dict.get('currency', 'SEASHELLS')),"price": float(trade_dict.get('price', 0.0)),"quantity": int(trade_dict.get('quantity', 0))}
                      safe_trades.append(safe_trade)
                 except Exception as dict_e:
                      print(f"Error converting trade dict {i}: {dict_e}")
                      safe_trades.append({"error": "trade dict conversion failed", "index": i})
            json.dump(sorted(safe_trades, key=lambda x: x.get('timestamp', 0)), output_stream, indent=2)
        except Exception as e:
